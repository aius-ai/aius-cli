...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "aius.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "aius.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "aius.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "aius.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["aius.json", "aius.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "aius.json"),
        path.join(tmp.path, "aius.jsonc"),
        path.join(project, "aius.json"),
        path.join(project, "aius.jsonc"),
      ])
    })
  })
