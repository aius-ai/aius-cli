...
import { ModelsDev } from "@aius-ai/core/models-dev"
...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    const error = yield* Provider.use
      .getModel(ProviderID.anthropic, ModelID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance(
  ...
  Effect.gen(function* () {
    yield* set("AIUS_API_KEY", "test-api-key")
    const providers = yield* list
    expect(providers[ProviderID.openrouter]).toBeDefined()

    // New model not in database should inherit api.url from provider. The
    // catalog routes through the AIUS edge proxy (AIUS_API_URL default), not
    // OpenRouter directly.
    const intellect = providers[ProviderID.openrouter].models["prime-intellect/intellect-3"]
    expect(intellect).toBeDefined()
    expect(intellect.api.url).toBe("http://localhost:8000/v1")
    ...
    expect(deepseek.api.url).toBe("http://localhost:8000/v1")
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".aius")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".aius")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
