...
it.instance("returns default native agents when no config", () =>
  Effect.gen(function* () {
    ...
    // "title" is no longer a client agent — session titling moved to the backend.
...
it.instance("plan agent denies edits except .aius/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".aius/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".aius", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.AIUS_TEST_HOME
      process.env.AIUS_TEST_HOME = test.directory
