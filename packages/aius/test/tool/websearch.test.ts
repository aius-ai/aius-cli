...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.AIUS_WEBSEARCH_PROVIDER

    try {
      process.env.AIUS_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.AIUS_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.AIUS_WEBSEARCH_PROVIDER
      else process.env.AIUS_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderID.openrouter, { exa: false, parallel: false })).toBe(false)
