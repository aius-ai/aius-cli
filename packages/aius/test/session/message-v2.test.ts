...
import { RunWsConnectionError } from "@/session/run-ws/errors"
...
describe("session.message-v2.fromError", () => {
  test("maps a rejected run-loop connection to an AuthError with its message", () => {
    const err = new RunWsConnectionError({ kind: "rejected", code: 1008, detail: "bad key or run loop disabled" })
    const result = MessageV2.fromError(err, { providerID })
    const data = result.data as { message?: string }
    expect(result.name).toBe("ProviderAuthError")
    expect(data.message).toBe("bad key or run loop disabled")
  })

  test("maps an unreachable run-loop connection to a retryable APIError", () => {
    const err = new RunWsConnectionError({ kind: "unreachable", detail: "could not connect" })
    const result = MessageV2.fromError(err, { providerID })
    const data = result.data as { message?: string; isRetryable?: boolean }
    expect(result.name).toBe("APIError")
    expect(data.message).toBe("could not connect")
