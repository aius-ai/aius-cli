...
const original = {
  ...
  envPassword: process.env.AIUS_SERVER_PASSWORD,
  envUsername: process.env.AIUS_SERVER_USERNAME,
}
const auth = { username: "aius", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.AIUS_SERVER_PASSWORD = original.AIUS_SERVER_PASSWORD
  Flag.AIUS_SERVER_USERNAME = original.AIUS_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.AIUS_SERVER_PASSWORD
  else process.env.AIUS_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.AIUS_SERVER_USERNAME
  else process.env.AIUS_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.AIUS_SERVER_PASSWORD = auth.password
  Flag.AIUS_SERVER_USERNAME = auth.username
  process.env.AIUS_SERVER_PASSWORD = auth.password
  process.env.AIUS_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.AIUS_SERVER_USERNAME = auth.username
  delete process.env.AIUS_SERVER_PASSWORD
  process.env.AIUS_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-aius-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-aius-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-aius-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-aius-ticket": "1" },
      })
