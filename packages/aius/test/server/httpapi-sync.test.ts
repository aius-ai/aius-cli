...
const originalWorkspaces = Flag.AIUS_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  mock.restore()
  Flag.AIUS_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("sync HttpApi", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        Flag.AIUS_EXPERIMENTAL_WORKSPACES = true
        const tmp = yield* TestInstance
        const headers = { "x-aius-directory": tmp.directory, "content-type": "application/json" }
        ...
        const response = yield* Effect.promise(() =>
          HttpApiApp.webHandler().handler(
            new Request(`http://localhost${SyncPaths.history}`, {
              method: "POST",
              headers: { "x-aius-directory": tmp.directory, "content-type": "application/json" },
