...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerAiusKeymap(keymap, renderer, config)
    ...
    return (
      <AiusKeymapProvider keymap={keymap}>
        <box />
      </AiusKeymapProvider>
    )
  }
...
test("mode-less bindings stay active when opencode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getAiusModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getAiusModeStack(keymap).push("autocomplete")
