...
This tracks legacy provider behavior from `packages/aius/src/provider/provider.ts` that still needs to be ported into the v2 provider plugins under `packages/aius/src/v2/plugin/provider/`. Keep entries checked only when v2 has equivalent behavior or when the item is intentionally skipped.
...
- [x] Aius unauthenticated paid-model filtering.
- [x] Aius public API key fallback.
...
- [x] Aius small model priority. Replaced by cheapest output cost selection.
