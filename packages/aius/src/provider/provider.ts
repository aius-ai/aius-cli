...
import { Hash } from "@aius-ai/core/util/hash"
import { Plugin } from "../plugin"
import { serviceUse } from "@aius-ai/core/effect/service-use"
import { type LanguageModelV3 } from "@ai-sdk/provider"
import * as ModelsDev from "@aius-ai/core/models-dev"
import { Auth } from "../auth"
import { Env } from "../env"
import { InstallationVersion } from "@aius-ai/core/installation/version"
...
import { optionalOmitUndefined } from "@aius-ai/core/schema"
...
const ENABLED_PROVIDER = ProviderID.openrouter
// AIUS edge proxy base URL. The client routes LLM traffic through aius_api
// (OpenRouter-wire-compatible: POST {AIUS_API_URL}/chat/completions with a
// Bearer AIUS key) instead of calling OpenRouter directly, so the OpenRouter
// key lives server-side and every call is logged to the Neo4j run model.
// The default is baked in at build time (see config/api-url.ts); override with
// AIUS_API_URL=https://api.aius.co/v1 for staging/prod or localhost for dev.
const AIUS_API_URL = apiBaseUrl()
// Run-loop WebSocket endpoint, derived from the proxy base at call time so
// tests (and AIUS_API_URL overrides) take effect. http(s) -> ws(s).
export const aiusRunWsUrl = () => apiBaseUrl().replace(/^http/, "ws") + "/runs/ws"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@aius-ai/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
  ...
  return {
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from aius.json provider config
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://github.com/aius-ai/aius",
            "X-Title": "aius",
            "X-Source": "aius",
          },
        },
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `aius/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `aius/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `aius auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `aius/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "aius",
          },
        },
      }),
    kilo: () =>
...
export class Service extends Context.Service<Service, Interface>()("@aius/Provider") {}
...
// The Aius default model and its standard-priced sibling. Injected into the
// OpenRouter catalog so they're always available even when models.dev hasn't
// published them yet. Cost is per-million tokens (models.dev convention).
const AIUS_DEFAULT_MODELS: ModelsDev.Model[] = [
  {
...
function ensureAiusDefaultModels(provider: ModelsDev.Provider): void {
  const models = provider.models as Record<string, ModelsDev.Model>
  for (const model of AIUS_DEFAULT_MODELS) {
    if (!models[model.id]) models[model.id] = model
  }
}

// Minimal AIUS provider shell for when models.dev is unreachable — keeps the
// injected Aius default models usable offline / during a catalog outage. Routes
// through the AIUS edge proxy (OpenRouter-wire-compatible); the internal id
// stays "openrouter" so the wire transforms and stored auth keep matching.
function openrouterFallbackProvider(): ModelsDev.Provider {
  return {
    id: "openrouter",
    name: "AIUS",
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const modelsDevAll = yield* modelsDevSvc.get()
        const modelsDev: Record<string, ModelsDev.Provider> = modelsDevAll.openrouter
          ? // Reuse the OpenRouter model catalog but route it through the AIUS
            // edge proxy: override name/api/env so the client talks to aius_api
            // (OpenRouter-wire-compatible) with an AIUS key. Clone the models map
            // so injecting Aius defaults doesn't mutate the ModelsDev service's
            // shared cache.
            {
              openrouter: {
                ...modelsDevAll.openrouter,
                name: "AIUS",
                api: AIUS_API_URL,
                env: ["AIUS_API_KEY"],
                models: { ...modelsDevAll.openrouter.models },
              },
            }
          : { openrouter: openrouterFallbackProvider() }
        // models.dev lags new releases. Aius's default model
        // (claude-opus-4.8-fast) and its non-fast sibling are injected here so
        // they're always selectable even before the catalog catches up.
        ensureAiusDefaultModels(modelsDev.openrouter)
        ...
        const configProviders = Object.entries(cfg.provider ?? {}).filter(
          ([providerID]) => providerID === ENABLED_PROVIDER,
        )
        const disabled = new Set(cfg.disabled_providers ?? [])
        const enabled = cfg.enabled_providers ? new Set(cfg.enabled_providers) : null

        function isProviderAllowed(providerID: ProviderID): boolean {
          if (providerID !== ENABLED_PROVIDER) return false
        ...
        for (const [id, provider] of Object.entries(auths)) {
          const providerID = ProviderID.make(id)
          if (providerID !== ENABLED_PROVIDER) continue
          if (disabled.has(providerID)) continue
        ...
        for (const [id, fn] of Object.entries(custom(dep)).filter(([id]) => id === ENABLED_PROVIDER)) {
          const providerID = ProviderID.make(id)
          if (disabled.has(providerID)) continue
...
// sort() ranks by priority.findIndex(...) DESC — so entries LATER in this
// array are MORE preferred. The Aius default (Opus 4.8 family) sits at the
// end; within a tie the secondary id-DESC sort puts the "-fast" id first.
const priority = [
  "gemini-3-pro",
  "big-pickle",
  "gpt-5",
