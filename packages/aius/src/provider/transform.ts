...
import type * as ModelsDev from "@aius-ai/core/models-dev"
...
export function options(input: {
  ...
  if (input.model.api.npm === "@openrouter/ai-sdk-provider" || input.model.api.npm === "@llmgateway/ai-sdk-provider") {
    ...
    // Aius default: Claude Opus 4.8 (fast or standard) and 4.7 all run with
    // high reasoning effort. The "fast" variant on OpenRouter is its own
    // model (anthropic/claude-opus-4.8-fast) — Anthropic's priority tier
    // baked into the model id — so we don't need provider.sort routing.
    if (
      input.model.api.id.includes("claude-opus-4-8") ||
      input.model.api.id.includes("claude-opus-4.8") ||
      input.model.api.id.includes("claude-opus-4-7") ||
      input.model.api.id.includes("claude-opus-4.7")
    ) {
      result["reasoning"] = { effort: "high" }
    }
