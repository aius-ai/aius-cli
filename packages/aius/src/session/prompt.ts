...
import * as EffectLogger from "@aius-ai/core/effect/logger"
...
import { AgentAttachment, FileAttachment, ReferenceAttachment, Source } from "@aius-ai/core/session-prompt"
...
export class Service extends Context.Service<Service, Interface>()("@aius/SessionPrompt") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Session titles are generated by the backend run loop now (it pushes a
    // `title` frame, persisted via the processor) — the client makes no LLM call.
    // The client only decides *whether* to ask, via `generateTitle` below.
    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          // Hard-stop at a human review gate. advance_stage tells the agent to
          // end its turn at a HITL stage, but it doesn't reliably comply — enforce
          // it so the session goes idle and the Continue gate appears instead of
          // running into the next stage. The agent's message must be the latest
          // (a user "continue" becomes latest on resume, so the loop proceeds).
          // Any orphaned trailing tool calls are cleaned by toWireMessages.
          if (lastAssistant && lastUser.id < lastAssistant.id) {
            const dsParked = yield* Effect.promise(async () => {
              if (!(await DSState.exists(ctx.directory).catch(() => false))) return false
              const ds = await DSState.load(ctx.directory).catch(() => undefined)
              return !!ds && DSStage.isHITL(ds.current_stage)
            })
            if (dsParked) {
              yield* slog.info("exiting loop at HITL review gate")
              break
            }
          }
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const result = yield* handle.process({
              ...
              // Ask the backend to title this conversation once, on the first step
              // of an untitled top-level session. Replaces the old client-side LLM
              // title call; the backend pushes a `title` frame.
              generateTitle: step === 1 && !session.parentID && Session.isDefaultTitle(session.title),
            })
