...
import { RunWsConnectionError } from "./run-ws/errors"
...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof RunWsConnectionError:
      // Bad key / run loop disabled reads as an auth problem; a dropped or
      // unreachable socket is an API/connection problem (unreachable is worth a
      // retry once the server is up).
      return e.kind === "rejected"
        ? new AuthError({ providerID: ctx.providerID, message: e.message }, { cause: e }).toObject()
        : new APIError(
            {
              message: e.message,
              isRetryable: e.kind === "unreachable",
              metadata: { code: String(e.code ?? ""), kind: e.kind },
            },
            { cause: e },
          ).toObject()
