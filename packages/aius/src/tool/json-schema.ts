...
function normalize(value: unknown, options: { stripNull?: boolean } = {}): unknown {
  ...
  // Anthropic (and OpenRouter routing into Anthropic) requires every object
  // tool-input schema to declare `required` — even an empty array — or it
  // refuses to route the request with "no endpoints found that support tool
  // use". Effect's schema generator only emits `required` when at least one
  // property is non-optional, so we add it back for the all-optional case.
