...
const MANAGED_PLIST_DOMAIN = "ai.aius.managed"

// Keys injected by macOS/MDM into the managed plist that are not Aius config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/aius"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "aius")
    default:
      return "/etc/aius"
  }
}
...
export function managedConfigDir() {
  return process.env.AIUS_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
