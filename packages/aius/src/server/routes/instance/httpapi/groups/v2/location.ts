...
import { Location } from "@aius-ai/core/location"
import { LocationServiceMap } from "@aius-ai/core/location-layer"
import { PluginBoot } from "@aius-ai/core/plugin/boot"
...
>()("@aius/ExperimentalHttpApiV2Location") {}

function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  const query = new URL(request.url, "http://localhost").searchParams
  return {
    directory: query.get("location[directory]") || request.headers["x-aius-directory"] || process.cwd(),
    workspaceID: query.get("location[workspace]") || request.headers["x-aius-workspace"],
  }
}
