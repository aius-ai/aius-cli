...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for aius serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AiusHttpApi)
...
export const PublicApi = AiusHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "aius",
    version: "1.0.0",
    description: "aius api",
