...
function GenericTool(props: ToolProps) {
  ...
  // Animate the icon while the tool is in flight (see index.tsx GenericTool).
  const running = createMemo(() => props.part.state.status === "running" || props.part.state.status === "pending")
  return (
    <Show
      when={output()}
      fallback={
        <InlineTool icon="⚙" pending="Working…" complete={toolComplete(props.part)} spinner={running()} part={props.part}>
...
function Skill(_props: ToolProps) {
  // Skill invocations are intentionally hidden from the UI. The agent still
  // runs them; the user just doesn't see the "→ Skill \"foo\"" marker.
  return null
}
