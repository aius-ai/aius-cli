...
import type { QuestionAnswer, QuestionRequest } from "@aius-ai/sdk/v2"
import { useSDK } from "../../context/sdk"
import { SplitBorder } from "../../component/border"
import { useTuiConfig } from "../../context/tui-config"
import { useBindings, useAiusModeStack } from "../../keymap"
...
export function QuestionPrompt(props: { request: QuestionRequest }) {
  ...
  const modeStack = useAiusModeStack()
  ...
  // A danger question (e.g. a multi-hour run) renders red. `danger` isn't in the
  // frozen generated SDK type (the regen pipeline is broken post-fork), but the
  // server schema carries it and it arrives at runtime — read it defensively.
  const danger = createMemo(() => questions().some((q) => (q as { danger?: boolean }).danger === true))
  const accentColor = () => (danger() ? theme.error : theme.accent)
        ...
        <Show when={!single()}>
          <box flexDirection="row" gap={1} paddingLeft={1}>
                ...
                return (
                  ...
                  >
                    <text
                      fg={
                        isActive()
                          ? selectedForeground(theme, accentColor())
                          : isAnswered()
                            ? theme.text
                            : theme.textMuted
                      }
            ...
            <box
              ...
              paddingRight={1}
              backgroundColor={
                confirm() ? accentColor() : tabHover() === "confirm" ? theme.backgroundElement : theme.backgroundPanel
              }
            ...
            >
              <text fg={confirm() ? selectedForeground(theme, accentColor()) : theme.textMuted}>Confirm</text>
            </box>
          </box>
        </Show>
                  ...
                  return (
                    ...
                    >
                      <box flexDirection="row">
                        ...
                        <Show when={i() === 0 && !multi()}>
                          <text fg={theme.secondary}>{"  · default (Enter)"}</text>
                        </Show>
                        <Show when={!multi()}>
