...
import aius from "./theme/aius.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@aius-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "aius")
        draft.active = typeof active === "string" ? active : "aius"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.aius, store.mode)
    })
