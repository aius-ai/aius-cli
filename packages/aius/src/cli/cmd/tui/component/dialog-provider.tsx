...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@aius-ai/sdk/v2"
...
export function providerOptions(list: { id: string; name: string }[]): ProviderOption[] {
  return [
    ...pipe(
      list.filter((provider) => provider.id === "openrouter"),
      ...
      map((provider) => ({
        ...
        description: provider.id === "openrouter" ? "(API key)" : undefined,
        category: "Providers",
      })),
    ),
  ]
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in aius.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={
        {
          opencode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Aius Zen gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
          ...
          "opencode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Aius Go is a $10 per month subscription that provides reliable access to popular open coding models
                with generous usage limits.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://opencode.ai/zen</span> and enable Aius Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in aius.json to use it.`,
          })
          dialog.clear()
          return
        }
