...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; gradient?: boolean } = {}) {
  const ctx = props.shape ? build(props.shape) : DEFAULT
  const { theme } = useTheme()
  // Horizontal gradient across the logo width: column x → tint(secondary, primary, t)
  // where t = x / (totalWidth - 1). Used by the Home screen wordmark.
  const totalWidth = (ctx.FULL[0]?.length ?? 1) - 1
  const gradientInk = (x: number) => {
    if (!props.gradient) return undefined
    const t = totalWidth <= 0 ? 0 : Math.max(0, Math.min(1, x / totalWidth))
    return tint(theme.secondary, theme.primary, t)
  }
  ...
  const renderLine = (
    ...
    return Array.from(line).map((char, i) => {
      const ink = gradientInk(off + i) ?? baseInk
