...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from aius.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const aius = await aiusFiles(input)
  for (const file of aius) {
...
async function aiusFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "aius"),
    ...(await Filesystem.findUp(["aius.json", "aius.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "aius"))
  }
  if (Flag.AIUS_CONFIG) files.push(Flag.AIUS_CONFIG)
