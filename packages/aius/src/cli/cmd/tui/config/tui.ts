...
import type { TuiAttentionSoundName } from "@aius-ai/plugin/tui"
...
export class Service extends Context.Service<Service, Interface>()("@aius/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old aius.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.aius`
  // folders between cwd and home, and AIUS_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AIUS_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AIUS_TUI_CONFIG override, if set.
  if (Flag.AIUS_TUI_CONFIG) {
    const configFile = Flag.AIUS_TUI_CONFIG
  ...
  // 4. `.aius` directories (and AIUS_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".aius") || dir === Flag.AIUS_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".aius") && dir !== Flag.AIUS_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "aius.default",
