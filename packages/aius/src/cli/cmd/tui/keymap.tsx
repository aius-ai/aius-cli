...
export const AIUS_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const AIUS_MODE_KEY = "aius.mode"

export const AiusKeymapProvider = KeymapProvider
export const useAiusKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type AiusModeStack = ReturnType<typeof createAiusModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, AiusModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createAiusModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(AIUS_MODE_KEY, AIUS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(AIUS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(AIUS_MODE_KEY, stack.at(-1)?.mode ?? AIUS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? AIUS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(AIUS_MODE_KEY, undefined)
...
export function useAiusModeStack() {
  return getAiusModeStack(useAiusKeymap())
}

export function getAiusModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Aius mode stack is not registered for this keymap")
  return value
}
...
export function registerAiusKeymap(
  keymap: OpenTuiKeymap,
  renderer: CliRenderer,
  config: Pick<TuiConfig.Resolved, "keybinds" | "leader_timeout">,
) {
  const modeStack = createAiusModeStack(keymap)
