...
import { Flock } from "@aius-ai/core/util/flock"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".aius"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".aius", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".aius", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void; disposeTimeoutMs?: number }) {
  ...
  try {
    ...
    const records = Flag.AIUS_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.AIUS_PURE && config.plugin_origins?.length) {
