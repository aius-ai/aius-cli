import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@aius-ai/plugin/tui"
...
import type { useAiusKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useAiusKeymap>
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getAiusModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getAiusModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
