...
import { Catalog } from "@aius-ai/core/catalog"
import { ModelV2 } from "@aius-ai/core/model"
import { PluginV2 } from "@aius-ai/core/plugin"
import { ProviderPlugins } from "@aius-ai/core/plugin/provider"
import { OpenRouterPlugin } from "@aius-ai/core/plugin/provider/openrouter"
  ...
  it.effect("filters OpenRouter's gpt-5 chat alias", () =>
    Effect.gen(function* () {
      ...
      yield* load((catalog) => {
        ...
        catalog.provider.update(ProviderV2.ID.make("openai"), () => {})
        for (const item of [
      ...
      expect((yield* catalog.model.get(ProviderV2.ID.make("openai"), ModelV2.ID.make("openai/gpt-5-chat"))).enabled).toBe(true)
    }),
  )
