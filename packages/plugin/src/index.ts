...
export type PluginInput = {
  client: ReturnType<typeof createAiusClient>
  project: Project
  directory: string
  worktree: string
...
// The TUI-facing config the plugin contract exposes. The v2 SDK config model
// doesn't carry these TUI fields (theme/tui), and nothing outside this package
// consumes the full app-config shape, so it's defined locally rather than pulled
// from the SDK. Source of truth for these fields is aius's own `config` schema.
export type Config = {
  $schema?: string
  theme?: string
  tui?: {
